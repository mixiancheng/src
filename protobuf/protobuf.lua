...
--判断是否字段有初始化过(optional)
local function _AddCustomMethod(message_descriptor,message_meta)
    message_meta._member.HasProperty = function (self, property)
...
local function _AddCopyFromMethod(message_descriptor, message_meta)
    local LABEL_REPEATED = FieldDescriptor.LABEL_REPEATED
    local CPPTYPE_MESSAGE = FieldDescriptor.CPPTYPE_MESSAGE
...
--    _AddEqualsMethod(message_descriptor, message_meta)
    ...
    _AddCustomMethod(message_descriptor, message_meta)
    _AddCopyFromMethod(message_descriptor, message_meta)
end
